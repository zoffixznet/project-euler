...
{
    io->file($RECTS_FN)
        ->print( encode_json( CalcRects::calc_processed_rects() ) );
}
...
sub rec
{
    ...
    if ( @$prev_coords == 3 )
    {
                    ...
                    )
                {
                    ...
                    else
                    {
                        ...
                        }
                        {
                            ...
                            $count += $O_C[ vec( $v, $byte_end, 16 ) =
                                ( $val | $XEs[ $x_end & 0xF ] ) ] - $O_C[$val];
                        }
                    }
        ...
        $total_count += $count;
        print "Found Count = $count for ("
            . join( ",", map { $_->[0] } @$prev_coords )
            . ") for a total of TotalCount = $total_count\n";
    }
    else
    {
        my $dim = @$prev_coords;
