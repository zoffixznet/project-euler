...
sub r_bw
{
    ...
    {
        $r = $C[$max_b_to_take][$count_of_b][$temp_max_w_to_take][$count_of_w]
            //= do
        {
            my $ret = 0;
          ...
          # for my $new_b_delta (reverse(0 .. min($count_of_b, $max_b_to_take)))
            for my $new_b_delta ( 0 .. min( $count_of_b, $max_b_to_take ) )
            {
                ...
                {
                    $ret += r_bw(
                        $new_b_delta, $count_of_b - $new_b_delta,
                        $w_delta,     $count_of_w - $w_delta,
                    );
                }
            }
            $ret;
...
# TEST
is( bw_total( 0, 0 ), 1, "bw_total(1,1) == 2", );

# TEST
is( bw_total( 0, 1 ), 1, "bw_total(0,1) == 1", );

# TEST
is( bw_total( 1, 0 ), 1, "bw_total(1,0) == 1", );
